...
1. 將 HPRT TP80K 安裝到店內 Windows 電腦，並設為預設印表機。
2. 完全關閉 Chrome，再執行專案內的 `啟動膳雉坊列印模式.bat`。
3. 登入後台後保持「列印工作站」頁面開啟，並關閉電腦睡眠功能。
4. 新訂單 LINE 通知會出現「列印餐點小票」按鈕；手機點擊後，店內電腦會在數秒內接收並列印。
5. LINE 會回覆工作編號；後台列印工作站也會顯示接收紀錄。

Chrome 必須由批次檔以 `--kiosk-printing` 啟動，才能略過列印確認視窗。系統的「已送往預設印表機」表示工作已交給 Windows 列印流程，不代表印表機硬體已完成出紙；缺紙、離線或卡紙仍需查看印表機狀態。
